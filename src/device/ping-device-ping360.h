...
public:
    ...
    /**
     * @brief The transducer will apply the commanded settings. The sonar will reply with a `device_data` message. If the `transmit` field is 0, the sonar will not transmit after locating the transducer, and the `data` field in the `device_data` message reply will be empty. If the `transmit` field is 1, the sonar will make an acoustic transmission after locating the transducer, and the resulting data will be uploaded in the `data` field of the `device_data` message reply. To allow for the worst case response time the command timeout should be set to 4000 msec.
     *
     * @param mode - Operating mode (1 for Ping360)
     * @param gain_setting - Analog gain setting (0 = low, 1 = normal, 2 = high)
     * @param angle - Units: gradians; Head angle
     * @param transmit_duration - Units: microseconds; Acoustic transmission duration (1~1000 us)
     * @param sample_period - Units: eicosapenta-nanoseconds; Time interval between individual signal intensity samples in 25 ns increments (80 to 40000 == 2 to 1000 us)
     * @param transmit_frequency - Units: kilohertz; Acoustic operating frequency (500~1000 kHz). It is only practical to use say 650 to 850 kHz due to the narrow bandwidth of the acoustic receiver.
     * @param number_of_samples - Units: samples; Number of samples per reflected signal (supported values: 200~1200)
    ...
    /**
     * @brief **NEW (v1.1.0)** Extended `transducer` message with auto-scan function. The sonar will automatically scan the region between `start_angle` and `end_angle` and send `auto_device_data` messages as soon as new data is available. Send a line break to stop scanning (and also begin the autobaudrate procedure). Alternatively, a `motor_off` message may be sent (but retries might be necessary on the half-duplex RS485 interface).
     *
     * @param mode - Operating mode (1 for Ping360)
     * @param gain_setting - Analog gain setting (0 = low, 1 = normal, 2 = high)
     * @param transmit_duration - Units: microseconds; Acoustic transmission duration (1~1000 us)
     ...
     * @param transmit_frequency - Units: kilohertz; Acoustic operating frequency (500~1000 kHz). It is only practical to use say 650 to 850 kHz due to the narrow bandwidth of the acoustic receiver.
     * @param number_of_samples - Units: samples; Number of samples per reflected signal (supported values: 200~1200)
     * @param start_angle - Units: gradians; Head angle to begin scan sector for autoscan (0~399 = 0~360 degrees).
     * @param stop_angle - Units: gradians; Head angle to end scan sector for autoscan (0~399 = 0~360 degrees).
     * @param num_steps - Units: gradians; Number of 0.9 degree motor steps between pings for auto scan (1~10 = 0.9~9.0 degrees)
     * @param delay - Units: milliseconds; An additional delay between successive transmit pulses (0~100 ms). This may be necessary for some programs to avoid collisions on the RS485 USRT.
     */
