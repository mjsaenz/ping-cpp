...
public:
    ...
    /**
     * @brief Set the profile output configuration. (Open source firmware only)
     *
     ...
     * device, and the values in the reply match the values that we applied
     */
    bool set_oss_profile_configuration(uint16_t number_of_points, uint8_t normalization_enabled, uint8_t enhance_enabled, bool verify = true);
    ...
    struct {
        ...
        // If enabled, the profile data is scaled so that the maximum value aligns with the upper limit of 255. (0: Disabled, 1: Enabled)
        uint8_t normalization_enabled;
        // If enabled, the profile data will be unevenly scaled to enhance peak values. (0: Disabled, 1: Enabled)
        uint8_t enhance_enabled;
    } oss_profile_configuration_data;
