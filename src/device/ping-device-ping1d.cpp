...
void Ping1d::_handleMessage(const ping_message* message)
{
    switch (message->message_id()) {
        ...
        {
            const ping1d_oss_profile_configuration* message_oss_profile_configuration = static_cast<const ping1d_oss_profile_configuration*>(message);
            oss_profile_configuration_data.number_of_points = message_oss_profile_configuration->number_of_points();
            oss_profile_configuration_data.normalization_enabled = message_oss_profile_configuration->normalization_enabled();
            oss_profile_configuration_data.enhance_enabled = message_oss_profile_configuration->enhance_enabled();
        }
        break;
...
bool Ping1d::set_oss_profile_configuration(uint16_t _number_of_points, uint8_t _normalization_enabled, uint8_t _enhance_enabled, bool verify)
{
    ping1d_set_oss_profile_configuration message;
    message.set_number_of_points(_number_of_points);
    message.set_normalization_enabled(_normalization_enabled);
    message.set_enhance_enabled(_enhance_enabled);
    writeMessage(message);
    // Check if we have a reply from the device
    if (!request(Ping1dId::OSS_PROFILE_CONFIGURATION)) {
        return false;
    }
    // Read back the data and check that changes have been applied
    if (verify
        && (oss_profile_configuration_data.number_of_points != _number_of_points
        || oss_profile_configuration_data.normalization_enabled != _normalization_enabled
        || oss_profile_configuration_data.enhance_enabled != _enhance_enabled)) {
        return false;
    }
    return true;
}
